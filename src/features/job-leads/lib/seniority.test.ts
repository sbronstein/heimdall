import { inferSeniority, seniorityWeights } from '@/features/job-leads/lib/seniority';
import { seniorityLevelValues } from '@/lib/domain/types';

describe('inferSeniority', () => {
  it('returns c_suite/100 for Chief Data Officer', () => {
    expect(inferSeniority('Chief Data Officer')).toEqual({ level: 'c_suite', weight: 100 });
  });

  it('returns vp/85 for VP of Engineering', () => {
    expect(inferSeniority('VP of Engineering')).toEqual({ level: 'vp', weight: 85 });
  });

  it('returns vp/85 for Vice President, Product', () => {
    expect(inferSeniority('Vice President, Product')).toEqual({ level: 'vp', weight: 85 });
  });

  it('returns director/70 for Senior Director of Data (director wins over senior due to rule order)', () => {
    expect(inferSeniority('Senior Director of Data')).toEqual({ level: 'director', weight: 70 });
  });

  it('returns senior_manager/55 for Senior Manager (rule-order regression guard)', () => {
    expect(inferSeniority('Senior Manager')).toEqual({ level: 'senior_manager', weight: 55 });
  });

  it('returns senior_manager/55 for Head of Growth', () => {
    expect(inferSeniority('Head of Growth')).toEqual({ level: 'senior_manager', weight: 55 });
  });

  it('returns manager/40 for Engineering Manager', () => {
    expect(inferSeniority('Engineering Manager')).toEqual({ level: 'manager', weight: 40 });
  });

  it('returns senior_ic/30 for Senior Staff Engineer', () => {
    expect(inferSeniority('Senior Staff Engineer')).toEqual({ level: 'senior_ic', weight: 30 });
  });

  it('returns ic/20 for Software Engineer (roleWordPattern fallback)', () => {
    expect(inferSeniority('Software Engineer')).toEqual({ level: 'ic', weight: 20 });
  });

  it('returns entry_level/10 for Junior Analyst', () => {
    expect(inferSeniority('Junior Analyst')).toEqual({ level: 'entry_level', weight: 10 });
  });
