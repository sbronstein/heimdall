...
**In progress:** Phase 13 (Triage Connection-Date Filter) complete (2026-06-21) — an additive, client-side connection-year / two-year-range filter on the existing triage queue: a tested pure-helper module (`connection-year.ts`), a reusable `ConnectionYearFilter` button-bar owning the `connectionYearStart`/`connectionYearEnd` nuqs URL state, and live-count + filter-specific empty-state wiring in `triage-workflow.tsx`. No DB/RSC change (D-04); owner human-verified all four criteria; the control is Phase-14-ready (D-08). Next: Phase 14 (Campaign Builder UI).
