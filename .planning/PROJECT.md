...
**In progress:** Phase 16 (Email Generation Skill) complete (2026-06-22) — the `generate-outreach-emails <campaign-id>` Claude Code skill drains a campaign's `pending` emails: reads `generation-context` once (anti-N+1), confirms the count, generates a 5-email sample for owner approval, then drains the rest in durable chunks — authoring closeness-calibrated subject+body in Steve's voice, self-correcting LLM tells (blocking scrub), and writing each back via a single `PATCH .../generation` that now also advances `status='generated'` (gated through `canEmailTransition`, the phase's one app-code change). Per-email failures mark `failed` and continue. Verification 5/5; all 7 code-review findings fixed inline (jq-escaped write-backs, sample persistence, skill-local `steve-fact-bank.md`, route 400/empty-guard, ownership test, doc sample-gate). Phases 14 (Campaign Builder UI) and 15 (Review & Approval UI) shipped earlier. Next: Phase 17 (Gmail Drafting & Email Discovery Skill).
...
---
*Last updated: 2026-06-22 — v1.2 Phase 16 (Email Generation Skill) complete: the `generate-outreach-emails` skill (SKILL.md + voice-guide / heimdall-api / steve-fact-bank references) drains a campaign's pending emails into closeness-calibrated, LLM-tell-clean, anti-hallucinated subject+body pairs, written back via a single PATCH that also flips status to `generated` (gated by `canEmailTransition`). Verification 5/5; all 7 code-review findings fixed inline. Next: `/gsd:discuss-phase 17` (Gmail Drafting & Email Discovery). Earlier this milestone: Phase 13 (Triage Connection-Date Filter) complete: an additive client-side connection-year / two-year-range filter on the existing triage queue — pure tested helpers (`connection-year.ts`, 20 Vitest cases), a reusable `ConnectionYearFilter` button-bar owning the `connectionYearStart`/`connectionYearEnd` nuqs URL state (the exact param names Phase 14 will drive `GET /api/contacts` with, D-08), plus live-count and filter-specific empty-state wiring in `triage-workflow.tsx`. No RSC/DB query change (D-04); `tsc` clean, 8/8 must-haves verified, owner human-verified criteria #1–#4, all 4 code-review findings resolved. Phase 12 (API Routes) shipped the `/api/outreach-campaigns/` REST surface. Phase 11 shipped migration 0013 to live Neon. v1.0 (Phases 1–6) and v1.1 (Phases 7–10) archived in `.planning/milestones/`. Next: `/gsd:discuss-phase 14` (Campaign Builder UI).*
